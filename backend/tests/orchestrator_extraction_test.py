...
def main() -> None:
    ...
    try:
        ...
        orch.handle_chat(ChatRequest(message="Eduardo Freire Maia", conversation_id=conversation_id))
        ...
        assert req.lead_data is not None and req.lead_data.get("email") == "contato.eduardofmaia@gmail.com", (
        ...
        if conversation_id is not None:
            session.execute(text("DELETE FROM agent_events WHERE conversation_id = :cid"), {"cid": str(conversation_id)})
            session.execute(text("DELETE FROM messages WHERE conversation_id = :cid"), {"cid": str(conversation_id)})
            session.execute(text("DELETE FROM conversations WHERE id = :cid"), {"cid": str(conversation_id)})
        if lead_id is not None:
            session.execute(text("DELETE FROM geospatial_analysis WHERE lead_id = :lid"), {"lid": str(lead_id)})
